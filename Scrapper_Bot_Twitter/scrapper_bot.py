...
import pandas as pd
import utils
# from .utils import utils.init_driver, utils.get_last_date_from_csv, utils.log_search_page, utils.keep_scroling, utils.dowload_images
...
def scrape(since, until=None, words=None, to_account=None, from_account=None, mention_account=None, interval=5, lang=None,
    ...
    if resume:
        since = str(utils.get_last_date_from_csv(path))[:10]
        write_mode = 'a'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', type=argparse.FileType('r'),
                         help='File containing queries. Each query should be on a separate line.', default=None)
    ...
    parser.add_argument('--until', type=str,
                        help='Max date for search query. example : %%Y-%%m-%%d.', default=None)
