...
import pandas as pd
# import utils
from .utils_single import init_driver, get_last_date_from_csv, log_search_page, keep_scroling, dowload_images
...
def scrape(since, until=None, words=None, to_account=None, from_account=None, mention_account=None, interval=5, lang=None,
    ...
    #     if type(words) == str:
    #         words = words.split("//")
    #     words = [w[:1] for w in words]  # take the first four letters of each word
    #     words = '_'.join(words)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--until', type=str,
                        help='Max date for search query. example : %%Y-%%m-%%d.', required=False)
